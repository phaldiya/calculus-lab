...
export default function ThreeDEquationList() {
  const { state, dispatch } = useAppContext();
  const equations = state.threeDGraphing.equations;

  if (equations.length === 0) {
    return (
      <p className="text-[var(--color-text-secondary)] text-sm italic">
        No surfaces yet. Enter z = f(x, y) above to plot.
      </p>
    );
  }
      ...
      {equations.map((eq) => (
        <div key={eq.id} className="flex items-center gap-2 rounded-lg bg-[var(--color-surface-alt)] px-3 py-2">
          <button
            ...
            onClick={() => dispatch({ type: 'TOGGLE_3D_EQUATION', id: eq.id })}
          ...
          <button
            ...
            onClick={() => dispatch({ type: 'REMOVE_3D_EQUATION', id: eq.id })}
